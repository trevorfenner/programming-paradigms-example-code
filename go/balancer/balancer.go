...
type Request struct {
	fn func() int // the function to perform the work
	c  chan int   // the communication channel to link to the function
}

func requester(work chan Request) chan int {
	c := make(chan int)
	for {
		time.Sleep(time.Duration(rand.Int63n(int64(nWorker * 2 * time.Second)))) // a little delay
		// send request to the channel with a return path for the result
		work <- Request{op, c}
		<-c
	}
}
